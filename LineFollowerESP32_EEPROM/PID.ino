// ===========================================================================
// Kontrol PID line follower.
// program(): pola sensor -> error (lookup), hitung PID, perintah motor.
//   mode : 0=center, 1=right, 2=left   (Follow Line Mode)
//   mask : 0=semua, 1..3 = abaikan 1..3 sensor tiap sisi (Used Sensor)
// Logika & tabel dipertahankan dari versi asli (sudah teruji di lintasan).
// ===========================================================================

void program(int dataSensor, byte mode, int speed, byte pid, byte mask) {

  if      (mask == 1) dataSensor = dataSensor & 0b01111111111110;
  else if (mask == 2) dataSensor = dataSensor & 0b00111111111100;
  else if (mask == 3) dataSensor = dataSensor & 0b00011111111000;
  ...
  int16_t leftW   = constrain(speed - moveVal, -255, 255);
  int16_t rightW  = constrain(speed + moveVal, -255, 255);

  if (dataSensor == 0b11111111111111) stopMotor();   // semua sensor "on" -> garis hilang
  else                                pwmMotor(leftW, rightW);
